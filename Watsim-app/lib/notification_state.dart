...
class NotificationState extends ChangeNotifier {
   ...
   try {
     ...
     for (final n in data) {
       ...
       _notifications.add(AppNotification(
         id: noteData['id'] as String? ?? 'note_${DateTime.now().millisecondsSinceEpoch}',
         ...
         timestamp: noteData['createdAt'] != null
             ? DateTime.tryParse(noteData['createdAt'] as String) ?? DateTime.now()
             : DateTime.now(),
 ...
 void onProfileUpdated(Map<String, dynamic> user) {
  ...
  // Messaging — seeded with support conversation, messages loaded from backend
  final List<Conversation> _conversations = [
    Conversation(
      id: 'watsim_support',
      name: 'Watsim Support',
      iconCodePoint: 0xef72,
 ...
 void addUserMessage(String convId, String text, {MessageAttachment? attachment}) {
 addMessageToConversation(convId, AppMessage(
 ...
 // Update unread: user sent, so don't increment their own count
 final conv = getConversation(convId);
 ...
 // Auto-reply from support after 2s
 if (convId =='watsim_support') {
 if (_awaitingCashCollectionDetails) {
 // User has just sent their name/location/phone — confirm dispatch
 _awaitingCashCollectionDetails = false;
 Future.delayed(const Duration(seconds: 2), () {
 addMessageToConversation('watsim_support', AppMessage(
 id:'auto_cash_${DateTime.now().millisecondsSinceEpoch}',
 conversationId:'watsim_support',
 text:'Thank you! We have received your details.\n\n'
' A Watsim official is now on their way to your location to collect your cash.'
'Please have the exact amount ready.\n\n'
'Once the cash is delivered to our office, your wallet will be credited immediately and you will receive a confirmation notification.'
'If you need to reach us in the meantime, just reply here. Thank you!',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 conversationId:'watsim_support',
 text:'Thanks for reaching out! Our team has received your message and will assist you shortly. For urgent issues, please check our Help Center.',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 /// Creates a new peer-to-peer conversation (or returns existing one).
 /// Returns the conversation id.
 String startUserConversation(String phoneOrId, String displayName) {
 final id ='user_$phoneOrId';
 ...
 final depCount = _notifications.where((n) => n.id.startsWith('dep_')).length;
 ...
 conversationId:'watsim_support',
 text:'Your withdrawal of $amtStr has been initiated. You\'ll receive the funds in your mobile money account within a few minutes.',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 conversationId:'watsim_support',
 text:'Your transfer of $amtStr to $recipient has been initiated. The recipient will receive the funds in their mobile money account within a few minutes.',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 void onBnplPaymentMade(String productName, int amount, int installment, int total) {
 final amtStr = _fmt(amount);
 ...
 void onBnplOrderConfirmed(String productName, int monthly, int months, {String paymentFrequency ='Monthly'}) {
 ...
 conversationId:'watsim_support',
 text:'Your BNPL order for $productName has been confirmed! You\'ll pay $amtStr per $freqLabel for $durationLabel. Your first instalment has been collected. Enjoy your new device!',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 body:'Transferred $amtStr completed the payment. ${_fmt(refund)} refunded to your wallet.',
 type:'transaction',
 ...
 conversationId:'watsim_support',
 text:'Great news! Your transfer of $amtStr from $fromProduct has fully covered the remaining balance on $toProduct — it''s now 100% paid off! The excess ${_fmt(refund)} has been refunded to your Watsim wallet.',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 id:'txc_${DateTime.now().millisecondsSinceEpoch}',
 title:'$toProduct — Fully Paid!',
 body:'Transfer of $amtStr completed all payments. The product is yours!',
 type:'transaction',
 ...
 id:'msg_txc_${DateTime.now().millisecondsSinceEpoch}',
 conversationId:'watsim_support',
 text:'Your transfer of $amtStr from $fromProduct has completed all payments for $toProduct! No more instalments — the product is fully yours.',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 title:'Refund Issued',
 body:'Transfer applied. Excess ${_fmt(refund)} returned to your wallet.',
 type:'transaction',
 iconCodePoint: 0xe8d6,
 iconColor: 0xFF1A5F7A,
 timestamp: DateTime.now(),
 ...
 // Tracks whether we are currently waiting for the user to supply their
 // cash-collection details (name, location, phone). When true, the next
 // user message in watsim_support is treated as those details and triggers
 // the "on their way" confirmation reply.
 bool _awaitingCashCollectionDetails = false;
 ...
 body:'Your $amtStr cash deposit is pending. Please provide your details so we can dispatch a Watsim official.',
 type:'transaction',
 ...
 _awaitingCashCollectionDetails = true;

 Future.delayed(const Duration(seconds: 2), () {
 addMessageToConversation('watsim_support', AppMessage(
 id:'msg_cash_pend_${DateTime.now().millisecondsSinceEpoch}',
 conversationId:'watsim_support',
 text:'Hello! We have received your cash deposit request of $amtStr.\n\n'
...
'You can now use your funds to shop, transfer, or pay instalments.',
 ...
 body:'You requested $reqStr but $colStr was received. Your wallet has been updated accordingly.',
 type:'transaction',
...
'or reply here and our support team will assist you promptly. We apologise for any inconvenience.';
 ...
 body:'Your delivery request for $productName has been received. You will receive your product shortly.',
 type:'transaction',
 ...
 conversationId:'watsim_support',
 text:'Your delivery request for $productName has been confirmed! Our team is preparing your shipment and you will receive it shortly. Thank you for using WatSim!',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
 ...
 body:'$productName was successfully delivered. Thank you for using WatSim!',
 type:'transaction',
 iconCodePoint: 0xe876, // check_circle
 iconColor: 0xFF00A86B,
 timestamp: DateTime.now(),
 ...
 conversationId:'watsim_support',
 text:'$productName was successfully delivered to you! We hope you enjoy it. Thank you for using WatSim — it\'s been a pleasure serving you! 🎉',
 isMe: false,
 timestamp: DateTime.now(),
 ));
 });
